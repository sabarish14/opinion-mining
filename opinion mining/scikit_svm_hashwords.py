...
feature_file=open("hash_words.txt","r") 
class_file=open("hash_labels.txt","r")
features=feature_file.readlines()
#labels=[None for i in range(len(features))]
'''for i in range(len(features)):
    temp=features[i].split(",")
    labels[i]=temp[len(temp)-1]'''
...
for str in labels:
    ...
    i=i+1
kf = cross_validation.KFold(len(labels), n_folds=10,shuffle=False)
accuracy=0
